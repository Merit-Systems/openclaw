...
describe("reactMessageDiscord", () => {
  ...
  it("resolves plain emoji name via guild lookup", async () => {
    ...
    await reactMessageDiscord("chan1", "msg1", "fatbiden", {
      rest,
      token: "t",
      cfg: DISCORD_TEST_CFG,
    });
  ...
  it("passes through plain emoji name when guild lookup fails", async () => {
    const { rest, putMock, getMock } = makeDiscordRest();
  ...
  it("passes through name:id format without guild lookup", async () => {
    const { rest, putMock, getMock } = makeDiscordRest();

    await reactMessageDiscord("chan1", "msg1", "fatbiden:999", {
      rest,
      token: "t",
      cfg: DISCORD_TEST_CFG,
    });
