...
describe("reactMessageDiscord", () => {
  ...
  it("resolves custom emoji by bare guild emoji name", async () => {
    const { rest, getMock, putMock } = makeDiscordRest();
    getMock
      .mockResolvedValueOnce({ guild_id: "guild1" })
      .mockResolvedValueOnce([{ id: "123", name: "party_blob" }]);

    await reactMessageDiscord("chan1", "msg1", "party_blob", {
      rest,
      token: "t",
      cfg: DISCORD_TEST_CFG,
    });
