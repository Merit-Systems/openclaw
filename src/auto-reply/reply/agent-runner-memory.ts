...
export async function runMemoryFlushIfNeeded(params: {
  ...
  try {
    const provider = normalizeOptionalString(params.sessionCtx.Provider)?.toLowerCase();
    const to = normalizeOptionalString(params.sessionCtx.OriginatingTo);
    if (provider === "discord" && to) {
      ...
      const content = messages[Math.floor(Math.random() * messages.length)] ?? messages[0];
      memoryDeps
        .sendMessage({
          channel: "discord",
          to,
    ...
    // Best-effort notification only; never block compaction.
  }
